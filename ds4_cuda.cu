...
static int cuda_matmul_q8_0_tensor_labeled(ds4_gpu_tensor *out, const void *model_map, uint64_t model_size, uint64_t weight_offset, uint64_t in_dim, uint64_t out_dim, const ds4_gpu_tensor *x, uint64_t n_tok, const char *label) {
    ...
    if (n_tok <= 4u) {
        static int fused_printed = 0;
        if (!fused_printed) { fprintf(stderr, "ds4: using fused q8 kernel n_tok=%lu in_dim=%lu out_dim=%lu blocks=%lu\n", (unsigned long)n_tok, (unsigned long)in_dim, (unsigned long)out_dim, (unsigned long)blocks); fused_printed = 1; }
