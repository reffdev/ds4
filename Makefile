...
ROCM_ARCH ?= gfx1151
GPU_CFLAGS ?= -O3 -g -fno-finite-math-only -pthread -D__HIP_PLATFORM_AMD__ \
...
GPU_CC = $(NVCC)
GPU_CFLAGS = -O3 -g -lineinfo --use_fast_math $(NVCC_ARCH_FLAGS) -Xcompiler $(NATIVE_CPU_FLAG) -Xcompiler -pthread
GPU_LDLIBS = -lm -Xcompiler -pthread -L$(CUDA_HOME)/targets/sbsa-linux/lib -L$(CUDA_HOME)/lib64 -lcudart -lcublas
EXTRA_DEPS =
endif
...
help:
	...
	@echo "  make rocm ROCM_ARCH=gfxNNNN  Build ROCm for an AMD GPU (e.g. gfx1151)"
	@echo "  make cpu                 Build CPU-only ./ds4, ./ds4-server, ./ds4-bench, ./ds4-eval, and ./ds4-agent"
	@echo "  make test                Build and run tests"
	@echo "  make clean               Remove build outputs"
